function auc = plotroc_liblinear(y,x,params)
%plotroc draws the recevier operating characteristic(ROC) curve.
%/share/pkg/liblinear
%/share/pkg/liblinear/2.1/install/lib
%predict.mexa64
%train.mexa64
% addpath(genpath('/share/pkg/liblinear/2.1'));
...
elseif exist('params') && ~ischar(params)%ischar(['-s 0',' -c ', num2str(0.1)]);params=['-s 0',' -c ', num2str(0.1)]
    model = params;%x*model.w';
...
resu=params;
fold=3;%5;
if ~isempty(params) && ~isempty(regexp(params,'-v'))%cmd = ['-s 3 -v 3 -c ', num2str(10^log10c)];
...
for i=1:nr_fold % Cross training : folding
    ...
    train_ind = [1:l]';
    train_ind(test_ind) = [];
    model = train(prob_y(train_ind),prob_x(train_ind,:),[param]);%	param
    %,' -w1 ', num2str(length(prob_y(train_ind))/sum(prob_y(train_ind)==1)/2), ' -w2 ', num2str(length(prob_y(train_ind))/sum(prob_y(train_ind)==-1)/2)
    %[param,' -w1 ', num2str(length(prob_y(train_ind))/sum(prob_y(train_ind)==-1)/2), ' -w2 ', num2str(length(prob_y(train_ind))/sum(prob_y(train_ind)==1)/2)]
    [predict_label,mse,subdeci] = predict(prob_y(test_ind),prob_x(test_ind,:),model);%predict(prob_y(test_ind),prob_x(test_ind,:),model);
